...
async def process_video():
    try:
        ...
        with NamedTemporaryFile(delete=False) as temp_video_file:
            ...
            # Collect paths of saved best images
            best_image_paths = [f"static/best_image_{i}.jpg" for i in range(len(best_frames))]
            
        os.remove(temp_video_file_path)
        # Return paths of saved best images in the response
        return jsonify({"message": "Processing completed successfully.", "best_image_paths": best_image_paths}), 200
    except Exception as e:
